function count = MIST(input_name, properties)
...
% Test if the input is as xpected !
if nargin == 0
...
elseif nargin == 2
  ...
  if region_automatic && region_manually
    disp("User asked for a manually and automatically defined region of the video! Only a single option is possible!")
    return
  end
  ...
  disp("Too any input arguments! The program will only use the first two arguments")
end
...
if ~exist(inputFullFileName, 'file')
  ...
  if strcmpi(response, 'OK - choose a new movie.')
    [baseFileName, folderName, ~] = uigetfile('*.mp4');
